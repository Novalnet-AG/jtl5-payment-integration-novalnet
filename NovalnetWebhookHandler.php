...
class NovalnetWebhookHandler
{
    ...
    public function getOrderReference(): object
    {
        ...
        if ($shopOrder !== false && !empty($shopOrder->kBestellung)) {
    ...
    public function handleNnTransactionCredit(): void
    {
        ...
        $webhookComments = "";
        $webhookComments .= \PHP_EOL . sprintf($this->novalnetPaymentGateway->novalnetPaymentHelper->getNnLangTranslationText('jtl_novalnet_webhook_initial_execution', $this->languageCode), $this->parentTid, $creditAmount, $this->eventTid);
    ...
    public function webhookFinalprocess($webhookMessage, $orderStatusToUpdate = '', $updateWawiStatus = 'N', $isPaidNow = false)
    {
        ...
        if (strpos($this->eventData['transaction']['payment_type'], 'INVOICE') !== false) {
            ...
            if (($this->eventType == 'INSTALMENT' || $this->eventType == 'TRANSACTION_UPDATE' && in_array($this->eventData['transaction']['update_type'], array('DUE_DATE', 'AMOUNT_DUE_DATE')))  && (preg_match('/before(.*)/', $oldTransactionComment->cKommentar, $matches) || preg_match('/zum(.*)/', $oldTransactionComment->cKommentar, $matches))) {
                ...
                $dueDateRaw = $this->eventData['transaction']['due_date'] ?? null;
                $dueDateFormatted = !empty($dueDateRaw)
                    ? date('d.m.Y', strtotime($dueDateRaw))
                    : '';
                $oldTransactionComment->cKommentar = ($oldTransactionComment->kSprache == 1) ?  str_replace($matches[1], date('d.m.Y', strtotime($this->eventData['transaction']['due_date'])) . ' auf das folgende Konto', $oldTransactionComment->cKommentar) :  str_replace($matches[0], 'before ' . $dueDateFormatted, $oldTransactionComment->cKommentar);
            }
        ...
        $updateData = [
            'cKommentar'  => $webhookComments,
            'cAbgeholt'   => $updateWawiStatus
        ];
        if (!empty($orderStatusToUpdate)) {
            $updateData['cStatus'] = $orderStatusToUpdate;
        }
        if ($this->eventData['transaction']['status'] == 'CONFIRMED') {
            if ($isPaidNow) {
                $updateData['dBezahltDatum'] = 'NOW()';
            }
        }
        $this->novalnetPaymentGateway->novalnetPaymentHelper->performDbUpdateProcess('tbestellung', 'cBestellNr', $orderNo, $updateData);
