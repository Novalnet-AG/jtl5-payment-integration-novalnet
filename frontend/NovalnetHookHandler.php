            ...
            if (strpos($configuration->valueID, trim('testmode') ) && $configuration->valueID != 'novalnet_webhook_testmode') {
                ...
                if (!empty($novalnetTestmode)) {
                    ...
                    foreach($paymentTestmodeArrs as  $key => $value) {
                        ...
                        $paymentId = Shop::Container()->getDB()->queryPrepared('SELECT cModulId FROM tpluginzahlungsartklasse WHERE cClassName LIKE :novalnetTestmodeId',[':novalnetTestmodeId' =>'"%' . $novalnetTestmodeId . '"' ] , 1);
...
HTML;
                    ...
                    if (strpos($payment->cModulId, 'novalnet')) {
                        ...
                        <script type='text/javascript' src='{$pluginPath}frontend/js/novalnet_cc_logo.min.js'  integrity="sha384-5avSRr3eO1lIGuF8RaVO5+9WTx4gcX6UiJ/zhquq8H+lNDCmSJfreetJEjywYT+g"></script>
        ...
        foreach ($smarty->tpl_vars['Zahlungsarten']->value as $payment) {
            if (strpos($payment->cModulId, 'novalnetapplepay')) {
                $nnCheckoutPageScript = <<<HTML
                        ...
                        <script type='text/javascript' src='{$pluginPath}paymentmethod/js/novalnet_applepay_hide.min.js'  integrity='sha384-9/f0gI6i0DY+8aAm1cOtCXAf4w6PYdggfJQKhRqw3JHP4i/cfdQrn/5f8u0DfeAN' defer></script>
HTML;
                // Method of connect on hook file to js file
