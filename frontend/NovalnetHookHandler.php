...
class NovalnetHookHandler
{
    ...
    public function accountPage(): void
    {
        if (!empty(Shop::Smarty()->tpl_vars['Bestellung'])) {
            ...
            $instalmentInfo = $this->novalnetPaymentGateway->getInstalmentInfoFromDb(Shop::Smarty()->tpl_vars['Bestellung']->value->cBestellNr, $lang);
		...
		foreach (Shop::Smarty()->tpl_vars['Zahlungsarten']->value as $payment) {
			 if (strpos($payment->cModulId, 'novalnet')) {
				if (in_array($step, ['Zahlung', 'Versand'])) {
					...
					$paymentFormRequestData = $this->novalnetPaymentGateway->generatePaymentParams();
					...
					if (!empty($_SESSION['novalnet']['order_amount'])) {
						$paymentFormRequestData['transaction']['amount'] = $_SESSION['novalnet']['order_amount'];
					}
