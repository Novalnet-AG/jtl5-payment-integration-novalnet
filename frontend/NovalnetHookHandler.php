...
class NovalnetHookHandler
{
    ...
    public function accountPage(): void
    {
        ...
        if ($refObject) {
         $tplVars = $this->smartydata($refObject); 
        }
        $Zahlungsarten = Shop::Smarty()->tpl_vars['Bestellung']->value ?? ($tplVars['Bestellung']->value) ??  null;
        if (!empty($Zahlungsarten)) {
            ...
            $getAdditionalInfo         = Shop::Container()->getDB()->queryPrepared('SELECT cAdditionalInfo,cZahlungsmethode FROM xplugin_novalnet_transaction_details WHERE cNnorderid = :cNnorderid', [':cNnorderid' => $orderNo], 1);
            $order_details = Shop::Container()->getDB()->queryPrepared('SELECT  `cKommentar` FROM `tbestellung` WHERE  cBestellNr = :cBestellNr', [':cBestellNr' => $orderNo], 1);
            ...
            if(in_array($getAdditionalInfo ->cZahlungsmethode,['novalnet_prepayment','novalnet_guaranteed_invoice','novalnet_invoice','novalnet_instalment_invoice'])){
            ...
            Shop::Smarty()->assign('instalmentDetails', $instalmentInfo)
                ...
                ->assign('nnComemntsOrder', $order_details);
    ...
    /**
     * fetch data in smarty 
     *
     * @return array
     */
    public function smartydata($refObject){
    ...
    public function contentUpdate(): void
    {
        ...
        if (Shop::getPageType() === \PAGE_BESTELLVORGANG) {
            ...
            $this->displayTranscationdetails($smarty);
        }
    }
    /**
     * Used for the frontend template customization wallet payment.
     *
     * @param  array $args
     * @return none
     */
    public function displaywalletpayments(array $args)
    {
                    ...
                    if (!empty($novalnetTestmode)) {
                        ...
                        foreach ($paymentTestmodeArrs as  $key => $value) {
                            ...
                            $paymentId = Shop::Container()->getDB()->queryPrepared(
                                'SELECT cModulId FROM tpluginzahlungsartklasse WHERE cClassName LIKE :novalnetTestmodeId',
                                [':novalnetTestmodeId' => '%' . $novalnetTestmodeId],
                                1
                            );
                            $splRejectedPaymentId = !empty($paymentId) ? str_replace('/', '', $paymentId->cModulId) : '';
                            $nnScriptHead = <<<HTML
...
HTML;
            ...
            if (!empty($nnCreditcardLogos)) {
                ...
                if (!empty($nnLogos)) {
                    ...
                    if ($smarty) {
                        $tplVars =  $this->smartydata($smarty);  
                    }
                    if (!empty($smarty->tpl_vars['Zahlungsarten']->value)) {
                        $Zahlungsarten = $smarty->tpl_vars['Zahlungsarten']->value;
                    } elseif (!empty($tplVars['Zahlungsarten']->value)) {
                        $Zahlungsarten = $tplVars['Zahlungsarten']->value;
                    }
                    if (!empty($Zahlungsarten) && is_iterable($Zahlungsarten)) {
...
HTML;
                        ...
                        <script type='text/javascript' src='{$pluginPath}frontend/js/novalnet_cc_logo.min.js' integrity="sha384-nbjFLRbO893N/vDWpzkeWb/sGZrLQtixbhwiBnBV7mAh9Tu69hetNrLh8/w+RREh" ></script>
...
HTML;
    ...
    /**
     * Display the  on success page
     *
     * @param  object  $smarty
     * @return none
     */
    public function displayTranscationdetails($smarty): void
    {
     ...
     * @return none
     */
    public function displayWalletPayment($args): void
    {
        ...
        if (!empty($positionArr)) {
            foreach ($positionArr as $positionDetails) {
                if (!empty($positionDetails->kArtikel && !empty($positionDetails->fVK[0]))) {
        ...
        $product = $args['smarty']->tpl_vars['Artikel']->value ?? ($args['oArtikel'] ?? null);
        // Product name and amount in products details page
        if (empty($cartDetails->PositionenArr) && !empty($args)) {
            $productName   = $product->cName ?? '';
            $productamount = $product->Preise->fVKBrutto ?? 0;
            $productPrice = round(($productamount ?? 0) * 100);
        ...
        $shopURL = Shop::getURL();
        // Set the product id
        $productId = $product->kArtikel ?? null;
        ...
        $iopath =  version_compare(\APPLICATION_VERSION, '5.4.0-rc.1', '>=')  ? '/io' : '/io.php';
        ...
        $nnWalletScript    .= <<<HTML
                                ...
                                <input type="hidden" name="nn_io_path" id="nn_io_path" value="{$iopath}">
...
HTML;
        ...
        if (!empty($walletPayments)) {
            $nnWalletScript .= <<<HTML
                    ...
                    <script type='text/javascript' src='{$pluginPath}frontend/js/novalnet_wallet.min.js' integrity="sha384-QCjTyv5mnBo/StPgTx69MTHKe8dQdmf8Kr3QizD0kR0XfTjkc0kHDG6PlqIXhP4g" defer></script>
                    <script type='text/javascript' src='{$pluginPath}frontend/js/novalnet_wallet_helper.min.js' integrity="sha384-OQ3/GVXlbCHDSVpX04eDwUTZ78eEs7pofWY/27fi3QnczqCWHHN4wmghlp8iHKMj"></script>
            ...
            foreach ($Zahlungsarten as $payment) {
                if (strpos($payment->cModulId, 'novalnetapplepay')) {
                    $nnCheckoutPageScript = <<<HTML
                        ...
                        <script type='text/javascript' src='{$pluginPath}paymentmethod/js/novalnet_applepay_hide.js'defer></script>
HTML;
                    // Method of connect on hook file to js file
