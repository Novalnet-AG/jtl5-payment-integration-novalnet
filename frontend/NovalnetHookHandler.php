...
HTML;
    ...
    public function displayWalletPayment(array $args): void
    {
        ...
        $finalArticleDetails  = $shippingMethodDetails = $defaultShippingIdentifier = $cartEmpty = '';
        ...
        if(!empty($positionArr)) {
            ...
            foreach($availableCoupons as $coupon) {
                if(!empty($_SESSION[$coupon])) {
                    ...
                    $articleDetails[] = array('label' => $_SESSION[$coupon]->cName, 'amount' => floatval($couponAmount), 'type' => 'SUBTOTAL');
                }
            }
        ...
        if(empty($cartDetails->PositionenArr)) {
            $productName = $args['smarty']->tpl_vars['Artikel']->value->cName;
            $productPrice = round($args['smarty']->tpl_vars['Artikel']->value->Preise->fVKBrutto * 100);
...
HTML;
        ...
        $nnWalletScript .= <<<HTML
                    ...
                    <script type='text/javascript' src='{$pluginPath}frontend/js/novalnet_wallet_helper.js'></script>
                    <script type='text/javascript' src='{$pluginPath}frontend/js/novalnet_wallet.js' defer></script>
HTML;
