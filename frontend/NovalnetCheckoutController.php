...
class NovalnetCheckoutController
{
    ...
    public function renderSummaryPage()
    {
        ...
        foreach ($required_fields as $key) {
            if ('firstName' === $key) {
                if (empty($_SESSION['Kunde']->cVorname)) {
                    $address = 'Billing';
                    $error = 'error';
                }
                if (empty($_SESSION['Lieferadresse']->cVorname)) {
                    $address = 'Shipping';
                    $error = 'error';
                }
            } elseif ('lastName' === $key) {
                if (empty($_SESSION['Kunde']->cNachname)) {
                    $address = 'Billing';
                    $error = 'error';
                }
                if (empty($_SESSION['Lieferadresse']->cNachname)) {
                    $address = 'Shipping';
                    $error = 'error';
                }
            } elseif ('addressLines' === $key) {
                if (empty($_SESSION['Kunde']->cStrasse)) {
                    $address = 'Billing';
                    $error = 'error';
                }
                if (empty($_SESSION['Lieferadresse']->cStrasse)) {
                    $address = 'Shipping';
                    $error = 'error';
                }
            } elseif ('postalCode' === $key) {
                if (empty($_SESSION['Kunde']->cPLZ)) {
                    $address = 'Billing';
                    $error = 'error';
                }
                if (empty($_SESSION['Lieferadresse']->cPLZ)) {
                    $address = 'Shipping';
                    $error = 'error';
                }
            } elseif ('locality' === $key) {
                if (empty($_SESSION['Kunde']->cOrt)) {
                    $address = 'Billing';
                    $error = 'error';
                }
                if (empty($_SESSION['Lieferadresse']->cOrt)) {
                    $address = 'Shipping';
                    $error = 'error';
                }
            } else {
        ...
        if (!empty($_SESSION['Lieferadresse'])) {
            ...
            $this->shippingMethods = $shippingService->getPossibleShippingMethods(
                $_SESSION['Kunde'],
            ...
            $this->packagings = $shippingService->getPossiblePackagings(
            ...
            if (!empty($this->shippingMethods)) {
                $shippingMethod = reset($this->shippingMethods);
        
                $_SESSION['AktiveVersandart'] = $shippingMethod->id;
                $_SESSION['Versandart']       = $shippingMethod->legacyObject;
            }
        }
