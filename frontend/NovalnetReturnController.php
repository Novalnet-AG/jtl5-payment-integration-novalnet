...
class NovalnetReturnController
{
    ...
    public function finalProcess(): void
    {
        if(!empty($_GET['tid']) && $_GET['payment_type'] == 'GOOGLEPAY') {
            ...
            $paymentInfo = Shop::Container()->getDB()->query('SELECT kZahlungsart, cModulId FROM tzahlungsart WHERE  cModulId LIKE "%' . strtolower($_SESSION['novalnetWalletResponse']['paymentType']) . '" ', 2);
            ...
            $paymentRequestData['transaction']['amount'] =  $_SESSION['novalnetWalletResponse']['transaction']['amount'];
    ...
    {
        // Do the further payment process based on the payment response
        $response = $this->novalnetPaymentGateway->validatePaymentResponse($order, $_SESSION['nn_'. $paymentKey .'_payment_response'], $paymentKey);
