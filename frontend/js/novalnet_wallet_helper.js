...
function onprocessCompletion(payLoad, bookingResult, paymentName) {
    if (payLoad.result.status == 'SUCCESS') {
        ...
        jQuery.ajax({
            url        : jQuery('#nn_shop_url').val() + jQuery('#nn_io_path').val(),
            type       : 'post',
            dataType   : 'json',
...
function onPaymentButtonClicked(clickResult) {
    if (jQuery('#nn_product_page').val() == 1) {
		...
		var text = (jQuery('#nn_page_language').val() == 'eng') ? 'demo' : 'gerdemo';
        // Loop through all form elements to check validation
        forms.find(':input').each(function () {
            if (!this.checkValidity()) {
                event.preventDefault();
				event.stopImmediatePropagation(); 
                // Remove any existing error messages
                $(this).closest('.form-group').find('div.form-error-msg').remove();
                // Append new error message
                $(this).closest('.form-group')
                    .addClass('has-error')
                    .append('<div class="form-error-msg w-100">' + text + '</div>');
            }
        });
        
        // Code for handling the form if validation passes
        var jQueryform = jQuery('#buy_form');
        
        // Check if the form is valid before adding to the basket
        if (jQueryform[0].checkValidity()) {
			if(jQuery('#nn_product_page').val() == 1) {
				var jQueryform  = jQuery('#buy_form');
