...
class NovalnetPostDataController
{
    ...
    public function addShippingAmountTocart(int $shippingId, array $shippingAddress, int $paymentMethodId = 0): void
    {
        ...
        $shippingService = Shop::Container()->get(ShippingService::class);

        $articleShippingCosts = $shippingService->getCustomShippingCostsByCart(
            $shippingAddress['countryCode'],
            new CustomerGroup(Frontend::getCustomerGroup()->getID()),
        ...
        foreach ($articleShippingCosts as $articleShippingCost) {
            Frontend::getCart()->erstelleSpezialPos(
                $articleShippingCost->cName,
                1,
                $articleShippingCost->fKosten,
                Frontend::getCart()->gibVersandkostenSteuerklasse($shippingAddress['countryCode']),
                C_WARENKORBPOS_TYP_VERSAND_ARTIKELABHAENGIG,
                false
            );
        }
        if (version_compare(\APPLICATION_VERSION, '5.4.0-rc.1', '>=')) {
            ...
            $jtlCheckoutController->init();
            $jtlCheckoutController->shippingMethodIsValid($shippingMethodId, ['kVerpackung' => $packagingIds]);
            $jtlCheckoutController->checkPaymentMethod($paymentMethodId);
        } else {
    ...
    public function novalnetShippingAddressUpdate(): string
    {
        ...
        if (!empty($this->request['params'][0]['novalnetWalletShippingAddress'])) {
            ...
            $possibleShippingMethods = Shop::Container()->getDB()->queryPrepared('select v.cName, v.fPreis, v.kVersandart, v.eSteuer from tversandart v, tversandartzahlungsart vz, tzahlungsart z WHERE vz.kZahlungsart = z.kZahlungsart AND vz.kVersandart = v.kVersandart AND  cModulId LIKE :paymentType AND v.cLaender LIKE :countryCode', [':paymentType' => '%' . strtolower($this->request['params'][0]['novalnetWalletShippingAddress']['paymentType']) . '%', ':countryCode' => '%' . $this->request['params'][0]['novalnetWalletShippingAddress']['countryCode'] . '%'], 2);
            ...
            if (!empty($possibleShippingMethods)) {
                ...
                $shippingDetails = [];
                $uniqueIdentifiers = []; // To track unique shipping methods
                ...
                foreach ($possibleShippingMethods as $shippingMethod) {
                    ...
                    // Check if this shipping method identifier is already added
                    if (!in_array($shippingMethod->cName, $uniqueIdentifiers, true)) {
                        $shippingDetails[] = [
                        ...
                        // Add the identifier to the unique set
                        $uniqueIdentifiers[] = $shippingMethod->cName;
                    }
                }
        ...
        if (!empty($positionArr)) {
            foreach ($positionArr as $positionDetails) {
                if (!empty($positionDetails->kArtikel)) {
                    ...
                    $productPrice = !empty($positionDetails->fVK) ? (floatval($positionDetails->fVK[0]) * 100) : 0;
    ...
    {
        $paymentMethodId = 0;
        $paymentInfo = Shop::Container()->getDB()->queryPrepared('SELECT kZahlungsart, cModulId FROM tzahlungsart WHERE cModulId LIKE :paymentType', [':paymentType' => '%' . strtolower($paymentType) . '%'],  2);
