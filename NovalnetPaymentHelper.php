...
class NovalnetPaymentHelper
{
    ...
    public function incoming_payments($order, $paymentName)
    {
        $incomingPayment           = new stdClass();
        $incomingPayment->fBetrag  = $order->fGesamtsummeKundenwaehrung;
        $incomingPayment->cISO     = $order->Waehrung->cISO;
        $incomingPayment->cHinweis = $_SESSION['nn_' . $paymentName . '_payment_response']['transaction']['tid'];
    ...
    public function getPaymentReferenceValues($paymentName): ?array
    {
        ...
        if (($customerDetails->kKunde) != '') {
            $storedValues = Shop::Container()->getDB()->queryPrepared('SELECT nNntid, cStatuswert, cTokenInfo FROM xplugin_novalnet_transaction_details WHERE cZahlungsmethode LIKE :paymentName  AND cMail = :cMail AND cSaveOnetimeToken = :cSaveOnetimeToken AND cTokenInfo != :cTokenInfo ORDER BY kId DESC LIMIT 3', [':paymentName' => '%' . $paymentName, ':cMail' => $customerDetails->cMail, ':cSaveOnetimeToken' => '1', ':cTokenInfo' => ''], 2);
     ...
     * @return int
     */
    public function getOrderAmount($order = null)
    {
        if ($order == null) {
     ...
     * @return none
     */
    public function performDbUpdateProcess(string $tableName, $keyName, $keyValue, array $object): void
    {


        if(!empty($object['cKommentar']))
        {
            $object['cKommentar'] = preg_replace(
                '/^(.*Alternativ[e]?\s.*QR[- ]Code.*|.*Alternatively.*QR code.*|.*novalnet_qrcode.*)$/mi',
                '',
                $object['cKommentar']
            );
        }
