...
<fieldset>
    ...
    <div class="nn_sepa">
        ...
        <div class="row" id="nn_load_new_form">
            ...
            {/if}
            <div class="form-group col col-7 nn_account_holder_field" role="group">                
                <input type="text" class="form-control" id="nn_sepa_account_holder" name="nn_sepa_account_holder" value= "{$firstName} {$lastName}" size="32" autocomplete="off">
                <label for="nn_sepa_account_holder" class="col-form-label pt-0 nn_account_holder_label">{$languageTexts.jtl_novalnet_account_holder_name}</label>
            </div>
           <div class="form-group col col-7 nn_iban_field" role="group">                
                <input type="text" class="form-control" id="nn_sepa_account_no" name="nn_sepa_account_no" size="32" autocomplete="off" onkeypress="return NovalnetUtility.checkIban(event, 'bic_div');" onkeyup="return NovalnetUtility.formatIban(event, 'bic_div');" onchange="return NovalnetUtility.formatIban(event, 'bic_div');" style="text-transform:uppercase;">
            ...
            <div class="form-group col col-7 nn_bic_field" id="bic_div" role="group" style="display:none;">                
                <input type="text" class="form-control" name="nn_sepa_bic" id="nn_sepa_bic" size="32" autocomplete="off" onkeypress="return NovalnetUtility.formatBic(event);" onchange="return NovalnetUtility.formatBic(event);">
                <label for="nn_sepa_bic" class="col-form-label pt-0 nn_bic_label">{$languageTexts.jtl_novalnet_sepa_bic}</label>
            </div>
        </div><br>
        ...
        <input type="hidden" id="novalnet_decimal_separator" name="novalnet_decimal_separator" value="{$decimalSeparator}">
        <input type="hidden" id="novalnet_thousand_separator" name="novalnet_thousand_separator" value="{$thousandSeparator}">
                ...
                <div class="col col-12">
                    <div class="form-group" role="group">
                        <select id="nn_instalment_cycle" name="nn_instalment_cycle" class="form-control">
                                ...
                                {assign var="cycleAmount" value=$value|regex_replace:'/[^0-9\.]/':''}
									{if empty($orderId)}
									...
									{else}
										<option value="{$key}" data-amount="{$value}" data-cycle-amount="{$cycleAmount/100}">{$key} x {$value} {$languageTexts.jtl_novalnet_instalment_per_month_text}</option>
									{/if}
                                {/foreach}
                        </select>
        ...
        <input id="nn_bic_invalid" type="hidden" value="{$languageTexts.jtl_novalnet_bic_field_invalid}" />
        <input id="nn_accountname_empty" type="hidden" value="{$languageTexts.jtl_novalnet_account_name_empty}" />
        <script type="text/javascript" src="{$pluginPath}paymentmethod/js/novalnet_payment_form.min.js"  integrity="sha384-XzW9i/tTwYQnRVtvQYwlGVkjHLgK0pqQrQOksEGeeOaZhKLOJG0mEQLplMDWMWk0"></script>
