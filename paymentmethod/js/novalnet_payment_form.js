...
jQuery(document).ready(function () {
    ...
    if (jQuery.inArray( payment_name, ['novalnet_sepa', 'novalnet_guaranteed_sepa', 'novalnet_instalment_sepa', 'novalnet_direct_debit_ach']) > -1) {
        ...
        var paymentFormId = jQuery('#nn_payment').closest('form').attr('id');
            jQuery('#'+paymentFormId).submit(function () {
                if (jQuery('#nn_load_new_form').css('display') !== 'none' && (jQuery('#nn_sepa_account_no').val() == '')) { 
                    ...
                    jQuery('#novalnet_sepa_error_alert').show(); 
                    setTimeout(function() {
						jQuery('#novalnet_sepa_error_alert').hide(); 
					}, 3000);
					jQuery('#novalnet_sepa_error_alert').removeClass('d-none'); 
                    jQuery('.submit_once').removeAttr('disabled');                  
                    return false;
                }  
				if (jQuery('#bic_div').css('display') !== 'none' && (jQuery('#nn_sepa_bic').val() == '')) { 
                    jQuery('#novalnet_sepa_error_alert').text(jQuery('#nn_bic_invalid').val());
                    jQuery('#novalnet_sepa_error_alert').show(); 
                    setTimeout(function() {
						jQuery('#novalnet_sepa_error_alert').hide(); 
					}, 3000);
                ...
                if (jQuery('#nn_sepa_account_holder').val() == '') { 
                    jQuery('#novalnet_sepa_error_alert').text(jQuery('#nn_accountname_empty').val());
                    jQuery('#novalnet_sepa_error_alert').show(); 
                    setTimeout(function() {
						jQuery('#novalnet_sepa_error_alert').hide(); 
					}, 3000);
                ...
                if (jQuery('#nn_load_new_form').css('display') !== 'none') {
					if(jQuery('#nn_ach_account_holder').val() == '' || jQuery('#nn_ach_account_no').val() == '' || jQuery('#nn_ach_routing_no').val() == '') { 
                        if(jQuery('#nn_ach_account_holder').val() == '') {
							jQuery('#novalnet_ach_error_alert').text(jQuery('#nn_account_holder_invalid').val());
							jQuery('#novalnet_ach_error_alert').show();
							setTimeout(function() {
								jQuery('#novalnet_ach_error_alert').hide(); 
							}, 3000);
							jQuery('#novalnet_ach_error_alert').removeClass('d-none'); 
							jQuery('.submit_once').removeAttr('disabled');   
						}
						if(jQuery('#nn_ach_account_no').val() == '') {
							jQuery('#novalnet_ach_error_alert').text(jQuery('#nn_account_no_invalid').val());
							jQuery('#novalnet_ach_error_alert').show();
							setTimeout(function() {
								jQuery('#novalnet_ach_error_alert').hide(); 
							}, 3000);
                        	...
                        	if(jQuery('#nn_ach_account_holder').val() == '') {
							jQuery('#novalnet_ach_error_alert').text(jQuery('#jtl_novalnet_account_name_empty').val());
							jQuery('#novalnet_ach_error_alert').show();
							setTimeout(function() {
								jQuery('#novalnet_ach_error_alert').hide(); 
							}, 3000);
						...
						if(jQuery('#nn_ach_routing_no').val() == '') {
							jQuery('#novalnet_routing_error_alert').text(jQuery('#nn_routing_no_invalid').val());
							jQuery('#novalnet_routing_error_alert').show();
							setTimeout(function() {
								jQuery('#novalnet_routing_error_alert').hide(); 
							}, 3000);
							jQuery('#novalnet_routing_error_alert').removeClass('d-none'); 
							jQuery('.submit_once').removeAttr('disabled');   
						}
					return false;
					}
        ...
        jQuery('#nn_instalment_cycle').on('change',function() {             
            ...
            for (instalmentCycle = 1; instalmentCycle <= jQuery(this).val(); instalmentCycle++) {
                if(instalmentCycle != jQuery(this).val())
                {
                    ...
                    var decimalSeparator = jQuery('#novalnet_decimal_separator').val();
					var thousandSeparator = jQuery('#novalnet_thousand_separator').val();
                    cycleInformation += '<tr><td>' + instalmentCycle + '</td><td>'+ formatMoney(lastCycleAmount, decimalSeparator, thousandSeparator) + ' '+ jQuery('#nn_order_currency').val()+'</td></tr>';
                }
            }                           
    ...
    // validate the birthdate field
     if (jQuery('#nn_show_dob').css('display') !== 'none' && ((jQuery.inArray( payment_name, ['novalnet_guaranteed_invoice', 'novalnet_guaranteed_sepa', 'novalnet_instalment_invoice', 'novalnet_instalment_sepa']) > -1) )) {
        ...
        jQuery('#'+ paymentFormId).on('submit', function () {
                var dobValue = $('#nn_dob').val();
                var datePattern = /^(0[1-9]|[12][0-9]|3[01])\.(0[1-9]|1[0-2])\.(?!0000)\d{4}$/;

                // Check if date format is valid and length is 10 characters (DD.MM.YYYY)
                if (!datePattern.test(dobValue)) {
                    $('#novalnet_dob_error_alert').text($('#novalnet_dob_empty').val());
                    $('#novalnet_dob_error_alert').show();
                    setTimeout(function() {
                        $('#novalnet_dob_error_alert').hide();
                    }, 3000);
                    $('#novalnet_dob_error_alert').removeClass('d-none');
                    $('.submit_once').removeAttr('disabled');
                    return false;
                }
                var birthDay = $('#nn_dob').val().split('.');
                var dob = new Date(birthDay[2], birthDay[1] - 1, birthDay[0]); // Note: year, month-1, day
                ...
                if (age < 18) {
                    $('#novalnet_dob_error_alert').text($('#novalnet_dob_invalid').val());
                    $('#novalnet_dob_error_alert').show();
                    setTimeout(function() {
                        $('#novalnet_dob_error_alert').hide();
                    }, 3000);
                ...
                jQuery('#novalnet_dob_error_alert').show(); 
                setTimeout(function() {
					jQuery('#novalnet_dob_error_alert').hide(); 
				}, 3000);
    ...
    // For ACH account and routing number field  validation
    jQuery('.nn_ach_account_no, .nn_ach_routing_no').on('input', function() {
    ...
    // For ACH account holder field  validation
    jQuery('.nn_ach_account_holder').on('input', function() {
        this.value = this.value.replace(/[^\p{L}\p{M} .\-']/gu, '');
    });
    jQuery('#nn_sepa_account_holder').on('input', function () {
        this.value = this.value.replace(/[^\p{L}\p{M} .\-']/gu, '');
    });
});
...
function formatMoney(amount, decimal, thousands, decimalCount = 2, ) {
  try {
