...
class NovalnetPaymentGateway
{
    ...
    public function getTransactionInformation(object $order, array $paymentResponse, string $paymentName): string
    {
        ...
        if(!empty($_SESSION['cPost_arr']['kommentar'])) {
            $userComments = $_SESSION['cPost_arr']['kommentar'];
        }
    ...
    public function getBankdetailsInformation(object $order, array $paymentResponse, ?string $lang = null): string
    {
        ...
        if ($paymentResponse['transaction']['status'] != 'ON_HOLD' && isset($paymentResponse['transaction']['due_date'])) {
            $invoiceComments = \PHP_EOL . sprintf($this->novalnetPaymentHelper->plugin->getLocalization()->getTranslation('jtl_novalnet_invoice_payment_transfer_duedate_comments', $lang), number_format($amount / 100 , 2, ',', ''),  $paymentResponse['transaction']['currency'], $paymentResponse['transaction']['due_date']);
        } else {
            $invoiceComments = \PHP_EOL . sprintf($this->novalnetPaymentHelper->plugin->getLocalization()->getTranslation('jtl_novalnet_invoice_payment_transfer_comment', $lang), number_format($amount / 100 , 2, ',', ''),  $paymentResponse['transaction']['currency']);
        }   
        ...
        $firstPaymentReference = (!empty($paymentResponse['instalment']['cycle_amount'])) ? 'jtl_novalnet_instalment_payment_reference' : 'jtl_novalnet_invoice_payments_first_reference';
    ...
    {
        $multibancoComments  = \PHP_EOL . sprintf($this->novalnetPaymentHelper->plugin->getLocalization()->getTranslation('jtl_novalnet_multibanco_reference_text'), number_format($paymentResponse['transaction']['amount'] / 100 , 2, ',', ''), $order->Waehrung->htmlEntity);
    ...
    public function insertOrderDetailsIntoNnDb(object $order, array $paymentResponse, string $paymentName): void
    {
        ...
        $insertOrder->nBetrag            = !empty($paymentResponse['transaction']['amount']) ? $paymentResponse['transaction']['amount'] : round($order->fGesamtsumme * 100, 2);
        $insertOrder->cAdditionalInfo    = !empty($paymentResponse['instalment']) ? json_encode($paymentResponse['instalment']) : (!empty($zero_amount_booking) ? json_encode($zero_amount_booking) : '');
     ...
     * @return array|null
     */
    public function getInstalmentInfoFromDb($orderNo, ?string $lang = null): ?array
    {
			...
			foreach($positionArr as $positionDetails) {
				if (!empty($positionDetails->kArtikel)) {
					...
					if ($isPaypalcart) {
						...
						$currencyConvertedProductPrice = $currencyConversionFactor * $productAmount;
