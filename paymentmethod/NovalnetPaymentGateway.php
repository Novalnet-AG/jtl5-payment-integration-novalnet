...
class NovalnetPaymentGateway
{
    ...
    public function generatePaymentParams(object $order, string $paymentName): array
    {
        ...
        $paymentRequestData['transaction'] = [
                                               ...
                                               'system_name'   => 'jtlshop',
                                               'system_version' => Version::parse(APPLICATION_VERSION)->getOriginalVersion() . '_NN12.1.0',
     ...
     * @return string
     */
    public function getArticleDetails(string $paymentName): string
    {
        // Get article details
        ...
        if (!empty($positionArr)) {
            ...
            if (!empty($_SESSION['Versandart']->cName)) {
                ...
                $articleDetails[] = array('label' => $_SESSION['Versandart']->cName, 'amount' => (int) $convertedOrderAmount, 'type' => 'SUBTOTAL');
            }
			...
			if(!empty($_SESSION['Bestellung']->GutscheinLocalized)) {
				$totalAmountValue 			= $_SESSION['Bestellung']->GutscheinLocalized;
				$totalAmount 				= preg_replace('/[^0-9,.]/', '', $totalAmountValue);
				$creditAmount 				= (float) str_replace(',', '.', $totalAmount);
				$convertedCreditAmount 		=  strval(round($creditAmount, 2) * 100);
				$articleDetails[] = array('label' => $this->novalnetPaymentHelper->plugin->getLocalization()->getTranslation('jtl_novalnet_credit_used'), 'amount' => -( $convertedCreditAmount ), 'type' => 'SUBTOTAL');
			}
        }
