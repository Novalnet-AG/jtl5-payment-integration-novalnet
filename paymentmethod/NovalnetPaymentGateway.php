...
class NovalnetPaymentGateway
{
    ...
    public function generatePaymentParams(object $order, string $paymentName): array
    {
        ...
        $paymentRequestData['transaction'] = [
                                               ...
                                               'system_name'   => 'jtlshop',
                                               'system_version' => Version::parse(APPLICATION_VERSION)->getOriginalVersion() . '_NN12.3.0',
    ...
    public function getBankdetailsInformation(object $order, array $paymentResponse, $lang = null): string
    {
        ...
        $invoiceComments .= \PHP_EOL . 'BANK: ' . $paymentResponse['transaction']['bank_details']['bank_name'];
    ...
    {        
        ...
        for ($storePos = 1; $storePos <= count($nearestStores); $storePos++) {
            ...
            if (!mb_check_encoding($nearestStores[$storePos]['street'], 'UTF-8')) {
				$cashpaymentComments .= \PHP_EOL . mb_convert_encoding($nearestStores[$storePos]['street'], 'UTF-8', 'ISO-8859-1');
			} else {
				$cashpaymentComments .= \PHP_EOL . $nearestStores[$storePos]['street'];
			}
