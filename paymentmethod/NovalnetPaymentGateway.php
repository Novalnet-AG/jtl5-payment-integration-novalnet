...
class NovalnetPaymentGateway
{
    ...
    public function getTransactionInformation(object $order, array $paymentResponse, string $paymentName): string
    {
        ...
        if (isset($paymentResponse['transaction']) && $paymentResponse['transaction']['status'] == 'CONFIRMED' && $paymentResponse['transaction']['amount'] == 0) {
    ...
    public function getBankdetailsInformation(object $order, array $paymentResponse, ?string $lang = null): string
    {
        ...
        $invoiceComments .= \PHP_EOL . $this->novalnetPaymentHelper->plugin->getLocalization()->getTranslation('jtl_novalnet_bank', $lang) . $paymentResponse['transaction']['bank_details']['bank_name'];
        $invoiceComments .= \PHP_EOL . $this->novalnetPaymentHelper->plugin->getLocalization()->getTranslation('jtl_novalnet_place', $lang) . $paymentResponse['transaction']['bank_details']['bank_place'];
    ...
    public function insertOrderDetailsIntoNnDb(object $order, array $paymentResponse, string $paymentName, ?array $txAdditonalInfo = null): void
    {
        ...
        if((isset($_SESSION['novalnet']['seamless_payment_form_response']['booking_details']['payment_action']) && $_SESSION['novalnet']['seamless_payment_form_response']['booking_details']['payment_action'] == 'zero_amount') ||  (isset($paymentResponse['transaction']) && $paymentResponse['transaction']['status'] == 'CONFIRMED' && $paymentResponse['transaction']['amount'] == 0)) {
			$zero_amount_booking = array ('zero_amount' => 1);
		}
		$instalment = array_key_exists('instalment', $paymentResponse) ? (array) $paymentResponse['instalment'] : [];
		$txInfo = isset($txAdditonalInfo) ? (array) $txAdditonalInfo : [];
		...
		$insertOrder->cAdditionalInfo = json_encode(array_merge($instalment, $txInfo));

		$insertOrder->nCallbackAmount = (!in_array($paymentName, ['invoice', 'prepayment', 'cashpayment', 'multibanco'])) ? (isset($paymentResponse['transaction']['amount']) ? $paymentResponse['transaction']['amount'] : 0) : 0;
        ...
        if(!empty($_SESSION['novalnet']['seamless_payment_form_response']['booking_details']['payment_ref_token'])) {
            ...
            if(!empty($_SESSION['novalnet']['seamless_payment_form_response']['booking_details']['pan_hash'])) {
                ...
                if(!empty($_SESSION['novalnet']['seamless_payment_form_response']['booking_details']['do_redirect'])) {
                    $paymentRequestData['transaction']['enforce_3d'] = 1;
                }
            }
