...
class NovalnetPaymentGateway
{
    ...
    public function generatePaymentParams(object $order, string $paymentName): array
    {
        ...
        $paymentRequestData['transaction'] = [
                                               ...
                                               'system_name'   => 'jtlshop',
                                               'system_version' => Version::parse(APPLICATION_VERSION)->getOriginalVersion() . '_NN12.1.2',
    ...
    public function getBankdetailsInformation(object $order, array $paymentResponse, $lang = null): string
    {
        ...
        if ($paymentResponse['transaction']['status'] != 'ON_HOLD') {
            $invoiceComments = \PHP_EOL . sprintf($this->novalnetPaymentHelper->plugin->getLocalization()->getTranslation('jtl_novalnet_invoice_payment_transfer_duedate_comments', $lang), number_format($amount / 100 , 2, ',', ''),  $order->Waehrung->htmlEntity, date('d.m.Y', strtotime($paymentResponse['transaction']['due_date'])));
        } else {
		...
		$invoiceComments .= ($paymentResponse['transaction']['payment_type'] == 'INSTALMENT_INVOICE') ? \PHP_EOL . $this->novalnetPaymentHelper->plugin->getLocalization()->getTranslation('jtl_novalnet_instalment_invoice_payments_single_reference_text', $lang) : \PHP_EOL . $this->novalnetPaymentHelper->plugin->getLocalization()->getTranslation('jtl_novalnet_invoice_payments_single_reference_text', $lang);
