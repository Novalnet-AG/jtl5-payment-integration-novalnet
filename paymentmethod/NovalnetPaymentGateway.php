...
class NovalnetPaymentGateway
{
    ...
    public function generatePaymentParams(object $order, string $paymentName): array
    {
        ...
        $paymentRequestData['transaction'] = [
            ...
            'system_name'   => 'jtlshop',
            'system_version' => Version::parse(APPLICATION_VERSION)->getOriginalVersion() . '_NN12.4.0',
     ...
     * @return bool
     */
    public function isTransactionRequiresAuthorizationOnly( float $orderAmount, string $paymentName): bool
    {
        ...
        if ($this->novalnetPaymentHelper->getConfigurationValues('payment_action', $paymentName)) {
            ...
            if (!empty($manualCheckLimit)) {
                 $manualCheckLimitRounded =  round((float) $manualCheckLimit);
                 $orderAmountRounded = round($orderAmount);
                return (preg_match('/^[0-9]*$/', $manualCheckLimit) && $orderAmountRounded > $manualCheckLimitRounded);
            }
    ...
    public function getTransactionInformation(object $order, array $paymentResponse, string $paymentName): string
    {
        ...
        $novalnetTxTid = $paymentResponse['transaction']['tid'] ?? $paymentResponse['tid'] ?? null;
    ...
    public function getBankdetailsInformation(object $order, array $paymentResponse, $lang = null): string
    {
        ...
        if (!empty($order->kBestellung)) {
            ...
            if ($paymentResponse['transaction']['status'] != 'ON_HOLD') {
                $dueDateRaw = $paymentResponse['transaction']['due_date'] ?? null;
                $dueDate = $dueDateRaw ? date('d.m.Y', strtotime($dueDateRaw)) : '';
                $invoiceComments = \PHP_EOL . sprintf(
                    $this->novalnetPaymentHelper->plugin->getLocalization()->getTranslation(
                        'jtl_novalnet_invoice_payment_transfer_duedate_comments',
                        $lang
                    ),
                    $totalamount,
                    $dueDate
        ...
        $invoiceComments .= \PHP_EOL . $this->novalnetPaymentHelper->plugin->getLocalization()->getTranslation('jtl_novalnet_invoice_payments_bank_name', $lang) . $paymentResponse['transaction']['bank_details']['bank_name'];
        $invoiceComments .= \PHP_EOL . $this->novalnetPaymentHelper->plugin->getLocalization()->getTranslation('jtl_novalnet_invoice_payments_bank_place', $lang) . $paymentResponse['transaction']['bank_details']['bank_place'];
        ...
        if (!empty($paymentResponse['transaction']['bank_details']['qr_image'])) {
            $invoiceComments .= \PHP_EOL . $this->novalnetPaymentHelper->plugin->getLocalization()->getTranslation('jtl_novalnet_qrcode', $lang);

            $qr_image = filter_var($paymentResponse['transaction']['bank_details']['qr_image'], FILTER_VALIDATE_URL);
            if ($qr_image) {

                $invoiceComments .= \PHP_EOL . sprintf(
                    '<img src="%s" alt="novalnet_qrcode" style="max-width:200px; margin-top:10px;">',
                    htmlspecialchars($qr_image, ENT_QUOTES, 'UTF-8')
                );
            }
        }
    ...
    public function insertOrderDetailsIntoNnDb(object $order, array $paymentResponse, string $paymentName, ?array $txAdditonalInfo = null): void
    {
        ...
        $insertOrder->nNntid             = $paymentResponse['transaction']['tid'] ?? $paymentResponse['tid'] ?? null;
        $insertOrder->cZahlungsmethode   = $paymentName;
        $insertOrder->cMail              = $customerDetails->cMail;
        $insertOrder->cStatuswert        = $paymentResponse['transaction']['status'] ?? $paymentResponse['status'] ?? null;
    ...
    public function saveRecentOrderPaymentData(string $paymentName, array $responsePaymentData): void
    {
        ...
        foreach ($getPaymentData as $paymentData) {
            ...
            if ($paymentName == 'sepa' && $tokenInfo['iban'] == $responsePaymentData['iban'] || ($paymentName == 'novalnet_paypal' && $tokenInfo['paypal_account'] == $responsePaymentData['paypal_account']) || ($paymentName == 'novalnet_cc' && $tokenInfo['card_number'] == $responsePaymentData['card_number']  && $tokenInfo['card_expiry_month'] == $responsePaymentData['card_expiry_month'] && $tokenInfo['card_expiry_year'] == $responsePaymentData['card_expiry_year']) || ($paymentName == 'novalnet_direct_debit_ach' && $tokenInfo['account_holder'] == $responsePaymentData['account_holder'] && $tokenInfo['routing_number'] == $responsePaymentData['routing_number'] && $tokenInfo['account_number'] == $responsePaymentData['account_number'])) {
     ...
     * @return string
     */
    public function convertCurrencyFormatter(int $kBestellung, string $amount)
    {
        $amount = $amount / 100;
        ...
        $totalAmount = html_entity_decode($totalAmount, ENT_QUOTES | ENT_HTML5, 'UTF-8');
        return $totalAmount;
    }
}
