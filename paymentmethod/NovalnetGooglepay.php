...
/**
 * Class NovalnetGooglepay
 * @package Plugin\jtl_novalnet\paymentmethod
 */
class NovalnetGooglepay extends Method
{
    /**
     ...
     * @var string
     */
    private $paymentName = 'novalnet_googlepay';

    /**
     * NovalnetGooglepay constructor.
     * 
    ...
    public function init(int $nAgainCheckout = 0): self
    {
        ...
        $this->name    = 'Novalnet Google Pay';
        $this->caption = 'Novalnet Google Pay';
    ...
    {
        $showPages  = $this->novalnetPaymentGateway->novalnetPaymentHelper->getConfigurationValues('novalnet_googlepay_button_display');
    ...
    public function handleAdditional(array $post): bool
    {
        ...
        $merchantId = !empty($this->novalnetPaymentGateway->novalnetPaymentHelper->getConfigurationValues('novalnet_googlepay_merchant_id')) ? $this->novalnetPaymentGateway->novalnetPaymentHelper->getConfigurationValues('novalnet_googlepay_merchant_id') : '';
        ...
        Shop::Smarty()->assign('pluginPath', $this->novalnetPaymentGateway->novalnetPaymentHelper->plugin->getPaths()->getBaseURL())
            ->assign('enforce', $this->novalnetPaymentGateway->novalnetPaymentHelper->getConfigurationValues('novalnet_googlepay_enforce_option'))
            ->assign('walletDetails', $walletDetails)
            ->assign('merchantId', $merchantId);
        return false;
    }
    ...
    public function preparePaymentProcess(Bestellung $order): void
    {
        ...
        $paymentRequestData['transaction']['payment_type'] = 'GOOGLEPAY';
        ...
        // If the payment processing requires a 3D authentication from the consumer, we redirect 
        if (($_SESSION['nn_wallet_doredirect'] == 'true') || ($EnforceOption == 'on')) {
    ...
    public function finalizeOrder(Bestellung $order, string $hash, array $args): bool
    {
        ...
        if (!empty($args['tid'])) {
            ...
            $this->novalnetPaymentGateway->validatePaymentResponse($order, $paymentResponseData, $this->paymentName);
            if ($args['status'] == 'SUCCESS' && $Zahlungsart != 0) {
