...
{     
    ...
    public function finalizeOrder(Bestellung $order, string $hash, array $args): bool
    {
        ...
        if (!empty($args['txn_secret'])) {
            ...
            if (!empty($args['tid'])) {
                ...
                $this->novalnetPaymentGateway->validatePaymentResponse($order, $paymentResponseData, $_SESSION['novalnet']['payment_key']);
    ...
    {   
        // Set the transcation details  token to session         
        if (isset($_SESSION['novalnet']['payment_response']) && $_SESSION['novalnet']['payment_response']['result']['status'] == 'SUCCESS') {
            $_SESSION['novalnet_transcation_details'] = $_SESSION['novalnet']['payment_response'];
        }
