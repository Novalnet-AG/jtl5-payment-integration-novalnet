...
class NovalnetWebhookHandler
{
    ...
    public function getOrderReference(): object
    {
        ...
        $novalnetOrder = Shop::Container()->getDB()->query('SELECT cNnorderid, cZahlungsmethode, cStatuswert, nBetrag, nNntid FROM xplugin_novalnet_transaction_details WHERE nNntid = "' . $this->parentTid . '"',  ReturnType::SINGLE_OBJECT);
        ...
        // Assign if payment module id is not exist in the orer object
        if(empty($order->Zahlungsart)) {
			$paymentMethodDetail = Shop::Container()->getDB()->query('SELECT cModulId FROM tzahlungsart WHERE cModulId LIKE "%novalnet%"', ReturnType::SINGLE_OBJECT);
			$order->Zahlungsart =  new stdClass();
			$order->Zahlungsart->cModulId = $paymentMethodDetail->cModulId;
		}
		
		// Check it may be zero amount process
        $txAdditonalInfo = Shop::Container()->getDB()->query('SELECT nNntid, cAdditionalInfo FROM xplugin_novalnet_transaction_details WHERE cNnorderid = "' . $orderNumberToSearch . '"', ReturnType::SINGLE_OBJECT);
    ...
    public function handleNnZeroAmountBooking(): void
    {
        ...
        $webhookComments = \PHP_EOL . sprintf($this->novalnetPaymentGateway->novalnetPaymentHelper->getNnLangTranslationText('jtl_novalnet_zero_amount_booking', $this->languageCode), $this->eventData['transaction']['amount'] / 100, $this->eventData['transaction']['currency'], $this->parentTid);
    ...
    {  
        ...
        if ($this->eventData['transaction']['update_type'] == 'STATUS') {
            ...
            $dueDateUpdateMessage = \PHP_EOL . sprintf($this->novalnetPaymentGateway->novalnetPaymentHelper->getNnLangTranslationText('jtl_novalnet_duedate_update_message', $this->languageCode), $this->eventData['transaction']['amount'] / 100, $this->eventData['transaction']['currency'], $this->eventData['transaction']['due_date']);
            ...
            $amountUpdateMessage = \PHP_EOL . sprintf($this->novalnetPaymentGateway->novalnetPaymentHelper->getNnLangTranslationText('jtl_novalnet_amount_update_message', $this->languageCode), $this->eventData['transaction']['amount'] / 100, $this->eventData['transaction']['currency'], date('d.m.Y'), date('H:i:s'));
    ...
    public function handleNnTransactionRefund(): void
    {
        if (!empty($this->eventData['transaction']['refund']['amount'])) {   
            ...
            if (!empty($this->eventData['transaction']['refund']['tid'])) {
                $webhookComments = \PHP_EOL . sprintf($this->novalnetPaymentGateway->novalnetPaymentHelper->getNnLangTranslationText('jtl_novalnet_new_tid_refund_execution', $this->languageCode), $this->parentTid, $this->eventData['transaction']['refund']['amount'] / 100, $this->eventData['transaction']['currency'], $this->eventData['transaction']['refund']['tid']);
            } else {
                $webhookComments = \PHP_EOL . sprintf($this->novalnetPaymentGateway->novalnetPaymentHelper->getNnLangTranslationText('jtl_novalnet_refund_execution', $this->languageCode), $this->parentTid, $this->eventData['transaction']['amount'] / 100, $this->eventData['transaction']['currency']); 
            }
    ...
    {
        $webhookComments = \PHP_EOL . sprintf($this->novalnetPaymentGateway->novalnetPaymentHelper->getNnLangTranslationText('jtl_novalnet_webhook_initial_execution', $this->languageCode), $this->parentTid, $this->eventData['transaction']['amount'] / 100, $this->eventData['transaction']['currency'], date('d.m.Y'), date('H:i:s'), $this->eventTid);
        ...
        if (in_array($this->eventData['transaction']['payment_type'], ['INVOICE_CREDIT', 'CASHPAYMENT_CREDIT', 'ONLINE_TRANSFER_CREDIT', 'MULTIBANCO_CREDIT'])) {
            ...
            $callbackDetails = Shop::Container()->getDB()->query('SELECT cAdditionalInfo, nCallbackAmount FROM xplugin_novalnet_transaction_details WHERE cNnorderid = "' . $this->orderDetails->cBestellNr . '"', ReturnType::SINGLE_OBJECT);
    ...
    public function handleNnChargeback(): void
    {
        if ($this->eventData['transaction']['payment_type'] == 'RETURN_DEBIT_SEPA') {
            $webhookComments = \PHP_EOL . sprintf($this->novalnetPaymentGateway->novalnetPaymentHelper->getNnLangTranslationText('jtl_novalnet_return_debit_execution_text', $this->languageCode), $this->parentTid, $this->eventData['transaction']['amount'] / 100, $this->eventData['transaction']['currency'], date('d.m.Y'), date('H:i:s'), $this->eventTid);

        } elseif ($this->eventData['transaction']['payment_type'] == 'REVERSAL') {
            $webhookComments = \PHP_EOL . sprintf($this->novalnetPaymentGateway->novalnetPaymentHelper->getNnLangTranslationText('jtl_novalnet_reversal_execution_text', $this->languageCode), $this->parentTid, $this->eventData['transaction']['amount'] / 100, $this->eventData['transaction']['currency'], date('d.m.Y'), date('H:i:s'), $this->eventTid);
        } else {
            $webhookComments = \PHP_EOL . sprintf($this->novalnetPaymentGateway->novalnetPaymentHelper->getNnLangTranslationText('jtl_novalnet_chargeback_execution_text', $this->languageCode), $this->parentTid, $this->eventData['transaction']['amount'] / 100, $this->eventData['transaction']['currency'], date('d.m.Y'), date('H:i:s'), $this->eventTid);
        }
    ...
    */
    public function handleNnInstalment(): void
    {
        if ($this->eventData['transaction']['status'] == 'CONFIRMED' && !empty($this->eventData['instalment']['cycles_executed'])) {
            ...
            $webhookComments = \PHP_EOL . sprintf($this->novalnetPaymentGateway->novalnetPaymentHelper->getNnLangTranslationText('jtl_novalnet_instalment_payment_execution', $this->languageCode), $this->parentTid, $this->eventData['transaction']['amount'] / 100, $this->eventData['transaction']['currency'], date('d.m.Y'), date('H:i:s'), $this->eventTid);
