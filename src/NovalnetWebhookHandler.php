...
class NovalnetWebhookHandler
{
    ...
    public function getOrderReference(): object
    {
        ...
        $novalnetOrder = Shop::Container()->getDB()->queryPrepared('SELECT cNnorderid, cZahlungsmethode, cStatuswert, nBetrag, nNntid FROM xplugin_novalnet_transaction_details WHERE nNntid  = :nNntid', [':nNntid' => $this->parentTid,], ReturnType::SINGLE_OBJECT);
    ...
    */
    ...
    {  
        $oldTransactionComment = Shop::Container()->getDB()->queryPrepared('SELECT cKommentar, kSprache FROM tbestellung WHERE cBestellNr  = :cBestellNr', [':cBestellNr' => $this->orderDetails->cBestellNr,], ReturnType::SINGLE_OBJECT);
    ...
    public function sendMailNotification(string $webhookMessage): void
    {
        ...
        foreach ($this->novalnetPaymentGateway->novalnetPaymentHelper->plugin->getMailTemplates()->getTemplatesAssoc() as $mailTemplate) {
            ...
            if ($mailTemplate->cModulId == 'novalnetwebhookmail' && $mailTemplate->cAktiv == 'Y') {
                ...
                $adminDetails = Shop::Container()->getDB()->queryPrepared('SELECT cMail from tadminlogin LIMIT :one', [':one' => '1'], ReturnType::SINGLE_OBJECT);
