...
class NovalnetBackendTabRenderer
{
    ...
    {
        $getOrder = $this->db->query('SELECT ord.kBestellung, ord.cKommentar, ord.kSprache FROM tbestellung ord JOIN xplugin_novalnet_transaction_details nov ON ord.cBestellNr = nov.cNnorderid WHERE cNnorderid = "' . $post['order_no'] . '"', ReturnType::SINGLE_OBJECT);
                
        $langCode = ($getOrder->kSprache == 1) ? 'ger' : 'eng';
