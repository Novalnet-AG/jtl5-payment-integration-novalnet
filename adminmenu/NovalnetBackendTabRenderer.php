...
class NovalnetBackendTabRenderer
{
    ...
    {
        $getOrder = $this->db->queryPrepared('SELECT  nov.cAdditionalInfo AS nnComments,  ord.kBestellung, ord.cKommentar, ord.kSprache FROM tbestellung ord JOIN xplugin_novalnet_transaction_details nov ON ord.cBestellNr = nov.cNnorderid WHERE cNnorderid = :cNnorderid', ['cNnorderid' => $orderNo], ReturnType::SINGLE_OBJECT);
             
        $getTransactionComments = json_decode($getOrder->nnComments, true);
         
        $langCode = ($getOrder->kSprache == 2) ? 'eng' : 'ger';
        
        $instalmentInfo = $this->novalnetPaymentGateway->getInstalmentInfoFromDb($orderNo, $langCode, $getOrder->kBestellung);
