jQuery(document).ready(function() {
        ...
        jQuery('button[name=speichern]').on('click', function(event){    
			...
			jQuery.each(['#novalnet_sepa_due_date'], function (index, element) {
				if (jQuery.trim(jQuery(element).val()) != '' && (isNaN(jQuery(element).val()) || jQuery(element).val() < 3 || jQuery(element).val() > 14)) {
        ...
        jQuery('#novalnet_postfinance_enablemode').closest('.nn-toggle-content').prepend(('<div class="form-group nn_additional_content"><span class="nn_content">' + jQuery('#nn_postfinance_api_configure').val() + '</span></div>'));

        jQuery('#novalnet_postfinance_card_enablemode').closest('.nn-toggle-content').prepend(('<div class="form-group nn_additional_content"><span class="nn_content">' + jQuery('#nn_postfinance_card_api_configure').val() + '</span></div>'));
